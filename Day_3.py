...
def rows_to_columns(binary_values):
    column_conversion = []
...
binary_columns = rows_to_columns(input_lines)
gamma = ''
...
while len(oxy_binaries) > 1:
    ...
    oxy_columns = rows_to_columns(oxy_binaries)
...
while len(co2_binary) > 1:
    ...
    co2_columns = rows_to_columns(co2_binary)

    i += 1

print("Submarine life support rating:",int(oxy_binaries[0], 2) * int(co2_binary[0], 2))
